...
namespace SLS.Widgets.Table {
  public sealed class Row: VisibleComponent, IPointerEnterHandler,
    ...
    protected override bool ShouldPostponeUpdate() {
      return base.ShouldPostponeUpdate() && !(this.datum.isDirty || this.datum.SafeHeight() < 0);
    }
#endif
    ...
    public void Refresh() {
      ...
      if(this.datum.animationStartTime > 0.0m) {
        decimal t = DateTime.Now.Ticks * Table.TicksToSecs;
        t -= this.datum.animationStartTime;
        if(t < this.table.rowAnimationDecimalDuration) {
          if(this.cDofadeCG != null) this.StopCoroutine(this.cDofadeCG);
          float currentAlpha = (float)(t / this.table.rowAnimationDecimalDuration);
          if(this.gameObject.activeInHierarchy)
            this.cDofadeCG = this.StartCoroutine(this.DofadeCG(this.table.rowAnimationDuration, currentAlpha, 1.0f));
          else
            this._cg.alpha = 1.0f;
        }
      }
    ...
    public void SetColor() {
      ...
      if(this.table.bodyScrollWatcher.isDragging) {
        if(this.datum != null && this.table.selectedDatumSet.Contains(this.datum))
      ...
      else if(this.datum != null && this.table.selectedDatumSet.Contains(this.datum))
        this.background.color = this.table.rowSelectColor;
