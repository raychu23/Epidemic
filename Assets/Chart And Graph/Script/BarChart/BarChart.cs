        ...
        void UpdateAnimations()
        {
            ...
            foreach (var pair in mSwitchEntries)
            {
                ...
                if(entry.time <=0)
                {
                    ...
                    bar.TopObject.transform.localPosition = entry.toPosition.Value;
                }
                else
                {
                    float factor = entry.time / entry.totalTime;
                    bar.TopObject.transform.localPosition = Vector3.Lerp(entry.toPosition.Value, entry.prevPosition, factor);
                }
            }
