...
public class AiTriggerCollider : MonoBehaviour
{
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        if (IsTagAllowed(other.tag) == true)
        {
			// Check if defender attacking other unit at the moment
			AiBehavior targetAi = other.gameObject.GetComponent<AiBehavior>();
			if (!(aiBehavior.tag == "Enemy" && targetAi != null && targetAi.tag == "Defender" && targetAi.currentState is AiStateAttack && (targetAi.currentState as AiStateAttack).target != aiBehavior.gameObject))
			{
				// Notify AI behavior about this event
