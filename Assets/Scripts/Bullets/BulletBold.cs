...
{
    // Damage amount
    [HideInInspector] public Medicine medicine;
    // Maximum life time
    ...
    // From which tower the bullet came from
    private Tower tower;
    // Aimed target
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
		if (IsTagAllowed(other.tag) == true)
		{
	        ...
	        Damage damageTaker = other.GetComponent<Damage> ();
	        if (damageTaker != null)
	        {
	            damageTaker.TakeDamage(medicine, tower);
	        }
		}
    }
