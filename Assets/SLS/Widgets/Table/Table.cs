...
namespace SLS.Widgets.Table {
  public class Table: UIBehaviour, ILayoutGroup {
    ...
    public static readonly decimal TicksToSecs = 0.0000001m;
    ...
    public bool alwaysMultiSelect = false;
    ...
    public void SetSelected(int x, int y, bool doCallback = true, bool animate = false) {
      Column column = null;
      Datum datum = null;
      if(x < this._columns.Count && x >= 0)
        column = this._columns[x];
      if(y < this.data.Count && y >= 0)
        datum = this.data[y];
      this.SetSelected(datum, column, doCallback, animate);
    }
    ...
    public void MoveSelectionUp(bool doCallback = true) {
      int x = this._columns.IndexOf(this._lastSelectedColumn);
      int y = this.data.IndexOf(this._lastSelectedDatum);
      y = y - 1;
      this.SetSelected(x, y, doCallback, true);
    }
    ...
    public void SetSelected(Datum d, Column c = null, bool doCallback = true, bool animate = false, bool setFocusIfInput = true) {
      ...
      if(this.selectedDatumSet == null)
        return;

      if(this.selectionMode != SelectionMode.MULTIROW && this.selectionMode != SelectionMode.MULTICELL) {
        HashSet<Datum> selectedDatumCopy = new HashSet<Datum>(this.selectedDatumSet);
        this.selectedDatumSet.Clear();
        this.selectedDatumColumnDict.Clear();
      ...
      if(d != null) {
        ...
        if(this.selectedDatumColumnDict.ContainsKey(d))
          datumColumnSet = this.selectedDatumColumnDict[d];
        else {
          datumColumnSet = new HashSet<Column>();
          this.selectedDatumColumnDict[d] = datumColumnSet;
        }

        this.selectedDatumSet.Add(d);
    ...
    public List<Datum> GetSelectedDatumList() {
      ...
      foreach(Datum d in this.data) {
        if(this.selectedDatumSet.Contains(d))
    ...
    private IEnumerator DoVertScrollUntilVisible(Datum d, int focusCellIdx = -1) {
      ...
      float numberOfOvershoots = 1f;
      bool? lastScrollUp = null;
      bool? lastScrollDown = null;
      ...
      Row r = null;
      while(scrollUp || scrollDown && this.isActiveAndEnabled && d == this.lastSelectedDatum) {
        scrollUp = false;
        scrollDown = false;
        ...
        if(scrollUp)
          this.bodyScroller.verticalNormalizedPosition += scrollAmount / numberOfOvershoots;// 0.01f;
        else if(scrollDown)
          this.bodyScroller.verticalNormalizedPosition -= scrollAmount / numberOfOvershoots;// 0.01f;
        ...
        if((lastScrollUp.HasValue && lastScrollUp.Value != scrollUp) || (lastScrollDown.HasValue && lastScrollDown.Value != scrollDown))
          numberOfOvershoots++;
    ...
    public Element GetSelectedElement() {
      if(this._lastSelectedColumn != null && this._lastSelectedDatum != null) {
        int x = this._columns.IndexOf(this._lastSelectedColumn);
        return this._lastSelectedDatum.elements[x];
      }
      return null;
    }
    ...
    public void ResetTable() {
      ...
      if(this.selectedDatumSet != null)
        ...
        this.selectedDatumSet = new HashSet<Datum>();

      if(this.selectedDatumColumnDict != null)
        this.selectedDatumColumnDict.Clear();
      else
        this.selectedDatumColumnDict = new Dictionary<Datum, HashSet<Column>>();
    }
    ...
    public void Initialize() {
      this.selectionCallback = null;
            ...
            Debug.Log("GOt inside Init func");
    ...
    public void SetGameObjectActiveLater(GameObject go, bool state) {
      if((go.activeInHierarchy && !state) || (!go.activeInHierarchy && state))
        this.StartCoroutine(this.DoSetGameObjectActiveLater(go, state));
    }
    ...
    public void FadeOverlay(float overTime, float v0, float v1, float delay = 0) {
      if(this.overlayIsHiding || !this.loadingOverlay.gameObject.activeInHierarchy)
        return;

      this.overlayIsHiding = true;
      this.StartCoroutine(this.DofadeOverlay(overTime, v0, v1, delay));
    }
    ...
    public void StartRenderEngine() {
      ...
      //StartCoroutine(_StartRenderEngine());
    ...
    IEnumerator _StartRenderEngine() {
      this._isRunning = true;
      if(this.hasHeader) {
        this.headerRow.datum = this.headerDatum;
        this.headerDatum.isDirty = true;
    ...
    public void SetBodyRectSizeLater(float s1, float s2) {
      if(this.cDoSetBodyRectSizeLater != null) this.StopCoroutine(this.cDoSetBodyRectSizeLater);
      this.cDoSetBodyRectSizeLater = this.StartCoroutine(this.DoSetBodyRectSizeLater(s1, s2));
    }
    ...
    private void RedrawTable() {
      if(this.isActiveAndEnabled && this.control != null) {
        ...
        if(this.cDoStartRenderLater != null) this.StopCoroutine(this.cDoStartRenderLater);
        this.cDoStartRenderLater = this.StartCoroutine(this.DoStartRenderLater());
      }
    }
    ...
    void OnDrawGizmosSelected() {
      ...
      Canvas c = this.GetComponentInParent<UnityEngine.Canvas>();
